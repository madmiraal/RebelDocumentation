...
Si ``true``, escala la imagen de arranque a la longitud total de la ventana cuando el motor se pone en marcha. Si ``false``, el motor lo dejará con el tamaño de píxeles predeterminado.
...
Ruta a una imagen usada como imagen de arranque.
...
Si ``true``, aplica un filtrado lineal al escalar la imagen (recomendado para obras de arte de alta resolución). Si ``false``, utiliza la interpolación del vecino más cercano (recomendado para el pixel art).
