...
- **G6DOF_JOINT_FLAG_ENABLE_LINEAR_LIMIT** = **0** --- Si ``set`` hay un movimiento lineal posible dentro de los límites dados.

- **G6DOF_JOINT_FLAG_ENABLE_ANGULAR_LIMIT** = **1** --- Si ``set`` hay un movimiento de rotación posible.

- **G6DOF_JOINT_FLAG_ENABLE_MOTOR** = **4** --- Si ``set`` hay un motor de rotación a través de estos ejes.

- **G6DOF_JOINT_FLAG_ENABLE_LINEAR_MOTOR** = **5** --- Si ``set`` hay un motor lineal en este eje que apunta a una velocidad específica.
...
Returns information about the current state of the 3D physics engine. See :ref:`ProcessInfo<enum_PhysicsServer_ProcessInfo>` for a list of available states. Only implemented for Rebel Physics.
...
**Note:** Only has an effect when using the Rebel Physics engine, not the default Bullet physics engine.
