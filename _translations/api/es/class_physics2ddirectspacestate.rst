...
Comprueba las intersecciones de una forma, dadas a través de un objeto :ref:`Physics2DShapeQueryParameters<class_Physics2DShapeQueryParameters>`, contra el espacio.

**Nota:** Este método no tiene en cuenta la propiedad ``motion`` del objeto. Las formas intersectadas se devuelven en una matriz que contiene diccionarios con los siguientes campos:

``collider``: El objeto que colisiona.

``collider_id``: El ID del objeto que colisiona.

``metadata``: Los metadatos de la forma que se intersecta. Estos metadatos son diferentes de :ref:`Object.get_meta<class_Object_method_get_meta>`, y se establecen con :ref:`Physics2DServer.shape_set_data<class_Physics2DServer_method_shape_set_data>`.

``rid``: El objeto que se intersecta es :ref:`RID<class_RID>`.

``shape``: El índice de la forma de la forma que colisiona.

El número de intersecciones puede ser limitado con el parámetro ``max_results``, para reducir el tiempo de procesamiento.
