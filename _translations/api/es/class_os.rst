...
Devuelve la fecha actual como un diccionario de claves: ``year``, ``month``, ``day``, ``weekday``, ``dst`` (Horario de verano).
...
Devuelve la fecha actual como un diccionario de claves: ``year``, ``month``, ``day``, ``weekday``, ``dst`` (horario de verano), ``hora``, ``minute``, ``second``.
...
Obtiene un diccionario de valores de tiempo correspondientes al tiempo de época UNIX dado (en segundos).

Los valores del diccionario devueltos serán los mismos que los de :ref:`get_datetime<class_OS_method_get_datetime>`, con la excepción del horario de verano, ya que no puede determinarse a partir de la época.
...
Devuelve el tiempo transcurrido en milisegundos desde que el motor se puso en marcha.
...
Devuelve la cantidad de tiempo transcurrido en microsegundos desde que el motor se puso en marcha.
...
Matar (terminar) el proceso identificado por el ID de proceso dado (``pid``), por ejemplo el devuelto por :ref:`execute<class_OS_method_execute>` en modo no bloqueante.

**Nota:** Este método también puede utilizarse para matar procesos que no fueron generados por el juego.
