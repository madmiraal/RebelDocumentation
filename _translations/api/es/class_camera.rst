...
- **DOPPLER_TRACKING_IDLE_STEP** = **1** --- Simular el `efecto Doppler <https://en.wikipedia.org/wiki/Doppler_effect>`__ rastreando las posiciones de los objetos que se cambian en el ``_process``. Los cambios en la velocidad relativa de esta cámara comparada con esos objetos afectan a la forma en que se percibe el Audio (cambiando el ``pitch shift`` del Audio).

- **DOPPLER_TRACKING_PHYSICS_STEP** = **2** --- Simular el `efecto Doppler <https://en.wikipedia.org/wiki/Doppler_effect>`__ rastreando las posiciones de los objetos que se cambian en el ``_physics_process``. Los cambios en la velocidad relativa de esta cámara comparada con esos objetos afectan a la forma en que se percibe el Audio (cambiando el ``pitch shift`` del Audio).
