...
Divide la string por una string de ``delimiter`` y devuelve un array de las substrings. El ``delimiter`` puede ser de cualquier longitud.

Si se especifica ``maxsplit``, define el número de divisiones a realizar desde la izquierda hasta ``maxsplit``. El valor por defecto de ``0`` significa que todos los elementos se dividen.
...
Si necesitas dividir string con reglas más complejas, usa la clase :ref:`RegEx<class_RegEx>` en su lugar.
