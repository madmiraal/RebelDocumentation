...
class in Rebel Engine as that's the one you use to code behavior and
...
save, Rebel Engine will save the image inside the ``.tscn`` scene file.
...
.. tabs::
 .. code-tab:: gdscript GDScript
    ...
    func _ready():
            var res = load("res://robi.png") # Rebel Engine loads the Resource when it reads the line.
 ...
 .. code-tab:: csharp
    ...
    {
        var texture = (Texture)GD.Load("res://robi.png"); // Rebel Engine loads the Resource when it reads the line.
...
.. tabs::
 .. code-tab:: gdscript GDScript
    ...
    func _ready():
            var res = preload("res://robi.png") # Rebel Engine loads the resource at compile-time
            get_node("sprite").texture = res
...
Like any Object in Rebel Engine, users can also script Resources. Resource scripts
...
- Resource auto-serialization and deserialization is a built-in Rebel Engine feature. Users do not need to implement custom logic to import/export a resource file's data.

- Resources can even serialize sub-Resources recursively, meaning users can design even more sophisticated data structures.

- Users can save Resources as version-control-friendly text files (\*.tres). Upon exporting a game, Rebel Engine serializes resource files as binary files (\*.res) for increased speed and compression.

- Rebel Editor's Inspector renders and edits Resource files out-of-the-box. As such, users often do not need to implement custom logic to visualize or edit their data. To do so, double-click the resource file in the FileSystem dock or click the folder icon in the Inspector and open the file in the dialog.

- They can extend **other** resource types besides just the base Resource.

Rebel Editor makes it easy to create custom Resources in the Inspector.
...
.. warning::
    ...
    work. Rebel Engine will not serialize the custom properties on the script subclass properly.

    In the example below, Rebel Engine would load the ``Node`` script, see that it doesn't
